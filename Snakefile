...
raw_data_dir = snakemake.config['raw-data-directory']
mzml_dir = snakemake.config['msconvert-directory']
mzmine_dir = snakemake.config['mzmine-directory']
...
rule all:
    input:
        ...
        metadata = snakemake.config['metadata-file'],
        ...
        mzml_files = expand(
            ...
            name=os.path.join(raw_data_dir, '{name}.d').name
        )

        # Require the results directory from running mzMine.
        mzmine_output = directory(snakemake.config['mzmine-directory'])
...
rule run_mzmine:
    input:
        batch_file = snakemake.config['mzmine-batch-file']
        mzml_files = rules.all.mzml_files
    output: rules.all.mzmine_output
